...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (btn) {
        ...
        function safeNavigateButton(btnEl, navigateFn, opts = {}){
            ...
            try { btnEl.disabled = true; btnEl.setAttribute('aria-busy','true'); } catch(e){}
            ...
            try { window.history.replaceState({step:'welcome'}, '', window.location.pathname); } catch(e) {}
            setTimeout(function(){
                try { navigateFn(); } finally { window[blockKey] = false; }
            }, delay);
        }

        btn.addEventListener('click', function () {
            safeNavigateButton(btn, function(){ window.location.href = '/pilih-layout'; });
        });
    ...
    if (AOS) {
        AOS.init({ duration: 1100, easing: 'ease-out-cubic', once: true, mirror: false });
        AOS.refresh();
    }
});
